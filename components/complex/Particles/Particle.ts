...
const isResizing = (particleCanvas: HTMLCanvasElement, connectCanvas: HTMLCanvasElement) => particleCanvas.classList.contains('is-resizing') && connectCanvas.classList.contains('is-resizing');

const detectResizeStart = () => {
  if (!isResizing(particleCanvas, connectCanvas)) {
    particleCanvas.classList.add('is-resizing');
    connectCanvas.classList.add('is-resizing');
...
const detectResizeFinish = debounce(() => {
  if (isResizing(particleCanvas, connectCanvas)) {
    createParticles();
    particleCanvas.classList.remove('is-resizing');
    connectCanvas.classList.remove('is-resizing');
  }
}, 250);
